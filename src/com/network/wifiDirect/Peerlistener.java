...
public class Peerlistener implements PeerListListener {
	...
	public void onPeersAvailable(WifiP2pDeviceList peers) {
		...
		// Log.d("HELLO", "PeerlistenerKör AsyncTask.execute");
		...
		for (WifiP2pDevice wifiP2pDevice : devs) {
			Log.d("HELLO", "Peerlistener - peersAvailable "
		...
		Log.d("HELLO",
				"Peerlistener - Är device empty? "
						+ String.valueOf(devs.isEmpty()));

	}

	public WifiP2pDevice getDevice() {
		if (device != null) {
			Log.d("HELLO", "Peerlistener - Hämtad adress "
					+ device.deviceAddress);
		} else {
			Log.d("HELLO", "Peerlistener - Device är null");
		}
		return device;
	}
